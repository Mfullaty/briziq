...
- **2026-07-14:** Payment Architecture. MVP will utilize TazaPay (for transactions $100-$14,000) and Escrow.com (for >$14,000) to handle cross-border payments and payouts. Buyers fund the respective escrow, and funds are disbursed to the seller upon release.
...
- **2026-07-14:** Backend Architecture. Supabase PostgreSQL is used as the database, accessed via Prisma ORM.
- **2026-07-14:** API Fetching. Client-side fetching via Axios and thin service layers calling Next.js API Routes is the standard pattern.
...
- **Custom Ledger / Banking Infrastructure:** Rejected for MVP. The platform will leverage existing compliance-heavy gateways (TazaPay and Escrow.com) rather than attempting to build proprietary money-transmitter architecture.
...
- **AI Document Verification (The OCR/LLM Stack):** The system utilizes Gemini, which is capable of high-fidelity OCR, cross-referencing dates, and flagging visual anomalies, piping into an admin queue for human sign-off.
...
- **Voice AI Stack:** Implementing FR-6 (Synthesized voice calls) will utilize Gemini to handle real-time conversational interactions.
