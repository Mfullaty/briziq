...
1. Read AGENTS.md.
2. **Read `docs/project_tracker.md`** to know what is completed, what is in progress, and what comes next. Do NOT start work on a feature that belongs to an epic whose prerequisites are incomplete. Do NOT re-implement something already marked `[x]`.
3. Read relevant project files in `docs/` to understand the product fully (e.g., PRD, Project Decisions, Design System).
4. Read `documentation/README.md` to fully understand the current codebase architecture.
5. Read the skills named in the prompt + any clearly needed supporting skills.
6. Inspect relevant code.
7. Ask a focused question only if there's real ambiguity.
8. Write a detailed prompt file in prompts/.
9. Ask: "I prepared the implementation prompt at prompts/<name>.md. Good to execute?"
10. Implement only after approval.
11. Run available checks.
12. Share exact test steps.
13. **Update `docs/project_tracker.md`**: mark completed items `[x]` with a date stamp, mark in-progress items `[/]`, and add an entry to the Update Log at the bottom.
